...
async function generateOgImage({
	...
	url,
	debug,
}: { html: JSX.Element; fetcher: Fetcher; url: URL; debug?: boolean }) {
...
async function getSessionDetailOgImageHtml({
	...
	const speakers = data.session_speakers_v2
		.map((sessionSpeaker) => {
			...
			return {
				...sessionSpeaker,
				speakerLogoUrl: supabase.storage
					.from("speakers")
					.getPublicUrl(avatarName.replaceAll(".webp", ".png")),
			};
		})
	...
	return (
		...
		>
			...
			>
				<div
					style={{
						fontSize: data.title.length > 30 ? "32px" : "40px",
						fontWeight: "bold",
						color: "#333333",
						maxHeight: "134px",
						display: "flex",
